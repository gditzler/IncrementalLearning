...
switch model.type
  case 'CART'
    ...
    % cart returns a cell arrary of strings; convert the predictions to a
    % vector of indices for the class being predicted. 
    predictions = zeros(length(predictions_raw),1);
    for m = 1:numel(predictions)
      predictions(m) = str2double(predictions_raw{m});
    end
end